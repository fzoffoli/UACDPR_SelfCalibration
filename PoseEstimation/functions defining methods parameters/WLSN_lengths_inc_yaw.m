function [S,j] = WLSN_lengths_inc_yaw(S,j,MyUACDPR,amperr_l,amperr_rollpitch,amperr_yaw,eps)
    ...
    S(j).Equations.F = @lengths_inc_yaw_equations;
    S(j).Equations.J = @lengths_inc_yaw_equations_dzita;
    ...
    S(j).Optimization.weigth_matrix = @weigth_matrix_lengths_inc_yaw;
    % Algorithm options
    S(j).Optimization.Options.Function_Tolerance =  (1+eps)*sqrt(double(MyUACDPR.CablesNumber)*amperr_l^2 + 2*amperr_rollpitch^2 + amperr_yaw^2);
