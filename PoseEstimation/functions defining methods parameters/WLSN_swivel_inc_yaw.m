function [S,j] = WLSN_swivel_inc_yaw(S,j,MyUACDPR,amperr_sigma,amperr_rollpitch,amperr_yaw,eps)
    ...
    % Algorithm options
    S(j).Optimization.Options.Function_Tolerance =  (1+eps)*sqrt(double(MyUACDPR.CablesNumber)*amperr_sigma^2 + 2*amperr_rollpitch^2 + amperr_yaw^2);
