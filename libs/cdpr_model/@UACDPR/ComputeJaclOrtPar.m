...
function UACDPR = ComputeJaclOrtPar(UACDPR)

J_d=UACDPR.AnalJac.Cables_P(1:nnz(UACDPR.DependencyVect),1:nnz(UACDPR.DependencyVect));
J_f=UACDPR.AnalJac.Cables_P(1:nnz(UACDPR.DependencyVect),(nnz(UACDPR.DependencyVect)+1):end);

% UACDPR.AnalJac.Cables_Port=[-inv(J_d)*J_f;
UACDPR.AnalJac.Cables_Port=[-linsolve(J_d,J_f);
                    eye(length(UACDPR.DependencyVect)-nnz(UACDPR.DependencyVect))];



UACDPR.AnalJac.Cables_Ppar=[inv(J_d);
                    zeros(length(UACDPR.DependencyVect)-nnz(UACDPR.DependencyVect),nnz(UACDPR.DependencyVect))];
...
% 
% J_d=UACDPR.GeomJac.Cables_P(1:nnz(UACDPR.DependencyVect),1:nnz(UACDPR.DependencyVect));
% J_f=UACDPR.GeomJac.Cables_P(1:nnz(UACDPR.DependencyVect),(nnz(UACDPR.DependencyVect)+1):end);
% 
...
UACDPR.GeomJac.Cables_Port = UACDPR.PermutMatrix*UACDPR.EndEffector.D*UACDPR.PermutMatrix'*UACDPR.AnalJac.Cables_Port;
UACDPR.GeomJac.Cables_Ppar = UACDPR.PermutMatrix*UACDPR.EndEffector.D*UACDPR.PermutMatrix'*UACDPR.AnalJac.Cables_Ppar;
