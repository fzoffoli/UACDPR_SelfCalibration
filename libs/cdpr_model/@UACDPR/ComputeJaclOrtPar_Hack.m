...
function UACDPR = ComputeJaclOrtPar_Hack(UACDPR)

J_d=UACDPR.AnalJac.Cables_P(1:nnz(UACDPR.DependencyVect),1:nnz(UACDPR.DependencyVect));
J_f=UACDPR.AnalJac.Cables_P((nnz(UACDPR.DependencyVect)+1):end,1:nnz(UACDPR.DependencyVect));

% UACDPR.AnalJac.Cables_Port=[-inv(J_d')*J_f';
%                     eye(length(UACDPR.DependencyVect)-nnz(UACDPR.DependencyVect))];
UACDPR.AnalJac.Cables_Port=null(UACDPR.AnalJac.Cables_Port')';
...
J_f=UACDPR.GeomJac.Cables_P((nnz(UACDPR.DependencyVect)+1):end,1:nnz(UACDPR.DependencyVect));

% UACDPR.GeomJac.Cables_Port=[-inv(J_d')*J_f';
%                     eye(length(UACDPR.DependencyVect)-nnz(UACDPR.DependencyVect))];
UACDPR.GeomJac.Cables_Port=null(UACDPR.GeomJac.Cables_Port')';


UACDPR.GeomJac.Cables_Ppar=[inv(J_d');
                    zeros(length(UACDPR.DependencyVect)-nnz(UACDPR.DependencyVect),nnz(UACDPR.DependencyVect))];
UACDPR.GeomJac.Cables_Ppar=pinv(UACDPR.GeomJac.Cables_Ppar);
