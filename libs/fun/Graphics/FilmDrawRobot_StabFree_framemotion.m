function [FF] = FilmDrawRobot_StabFree_framemotion(PosePList,PoseFrameList,obj)
 fig=figure
 hold on
 axis('equal')
...
% conj{3}=animatedline('MaximumNumPoints',2,'LineWidth',2,'Color','b');
% conj{4}=animatedline('MaximumNumPoints',2,'LineWidth',2,'Color','b');
...
grid on
view(-73.186854782826273,16.254267330080495)
...
for j=1:length(PosePList)
    ...
    Rotmatrix_vessel= obj.EndEffector.RotMatrix;
    vessel_points(:,1)=(PosePList(1:3,j)+Rotmatrix_vessel*[2;1;0.5]);
    vessel_points(:,2)=(PosePList(1:3,j)+Rotmatrix_vessel*[-2;1;0.5]);
    vessel_points(:,3)=PosePList(1:3,j)+Rotmatrix_vessel*([-2;-1;0.5]);
    vessel_points(:,4)=(PosePList(1:3,j)+Rotmatrix_vessel*[2;-1;0.5]);
j    
    %plot pulley frames
%     for i=1:obj.CablesNumber
    for i=5
...
norm(obj.EndEffector.GlobalAttachPoints{5}-(obj.Trasmission.Pulley{5}.CableExitPosition))+obj.Trasmission.Pulley{5}.TangencyAngle*.1;
        conj{1}=  drawline_animated((vessel_points(:,1)+vessel_points(:,2))./2,obj.EndEffector.GlobalAttachPoints{5},conj{1});
conj{2}=  drawline_animated((vessel_points(:,3)+vessel_points(:,4))./2,obj.EndEffector.GlobalAttachPoints{5},conj{2});
...
%     vessel_1=drawline_animated(vessel_points(:,4),vessel_points(:,1),vessel_1);
    ...
    vessel_2=drawline_animated(vessel_points(:,1)-Rotmatrix_vessel*[0;0;1],vessel_points(:,2)-Rotmatrix_vessel*[0;0;1],vessel_2);
    vessel_2=drawline_animated(vessel_points(:,3)-Rotmatrix_vessel*[0;0;1],vessel_points(:,4)-Rotmatrix_vessel*[0;0;1],vessel_2);
    vessel_2=drawline_animated(vessel_points(:,4)-Rotmatrix_vessel*[0;0;1],vessel_points(:,1)-Rotmatrix_vessel*[0;0;1],vessel_2);
%     vessel_2=drawline_animated(vessel_points(:,4)-Rotmatrix_vessel*[0;0;1],vessel_points(:,1)-Rotmatrix_vessel*[0;0;1],vessel_2);
    
    vessel_3=drawline_animated(vessel_points(:,1)-Rotmatrix_vessel*[0;0;1],vessel_points(:,1),vessel_3);
    vessel_3=drawline_animated(vessel_points(:,2),vessel_points(:,2)-Rotmatrix_vessel*[0;0;1],vessel_3);
    vessel_3=drawline_animated(vessel_points(:,3)-Rotmatrix_vessel*[0;0;1],vessel_points(:,3),vessel_3);
    vessel_3=drawline_animated(vessel_points(:,4),vessel_points(:,4)-Rotmatrix_vessel*[0;0;1],vessel_3);
